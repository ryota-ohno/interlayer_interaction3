...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        df_params=pd.read_csv('~/Working/interlayer_interaction3/{}/assets/step2_result.csv'.format(monomer_name))
        for index in df_params.index:
            ...
            if len(xyz)>0:
                for i in range(len(xyz)):
                    init_para_list.append([a_,b_,theta,R3,R4,xyz[i][0],xyz[i][1],np.round(xyz[i][2],1)-0.2,'NotYet'])
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','R3','R4','E','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
    else:
...
def listen(args):
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=10:##計算する層状の分子数
            ...
            E = (Ei01+Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4##隣接6分子　2パターン
            #### TODO
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    while True:
        E_list=[];cxyz_list=[]
        for cx in [cx_init_prev-0.2,cx_init_prev,cx_init_prev+0.2]:##刻み幅は適宜変化させる
            for cy in [cy_init_prev-0.2,cy_init_prev,cy_init_prev+0.2]:
                 for cz in [cz_init_prev-0.2,cz_init_prev,cz_init_prev+0.2]:  
